...
class BayesClassifier:
    ...
    def classify(self, text: str) -> str:
        ...
        # individual feature
        posWordTotal = sum(self.pos_freqs.values())
        negWordTotal = sum(self.neg_freqs.values())
        ...
        for token in tokens:
            try: 
                posProp = posProp + math.log( ((self.pos_freqs[token] + 1)/posWordTotal) )
                negProp = negProp + math.log( ((self.neg_freqs[token] + 1)/negWordTotal) )
            except KeyError:
                posProp = posProp + math.log( (1/posWordTotal) )
                negProp = negProp + math.log( (1/negWordTotal) )
